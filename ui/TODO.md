...
- [ ] **[Высокий]** Анимация раздачи не отыгрывается: `LobbyScreen` передаёт пост-раздачное состояние как `initialState`, и в `_onData` для первого сообщения `prev == null`. Синтезировать пре-раздачный `prev` (пустые руки/полная колода) при старте `OnlineGameScreen` и проигрывать переход к `initialState` через `_animateChanges`.
